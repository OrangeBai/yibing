...
def load_all_data_new(refresh=False) -> pd.DataFrame:
    ...
    if refresh or not ml.has_symbol('data/quote_new'):
        files = [os.path.join(dirpath, f) for dirpath, dirnames, file_names in os.walk(QUOTE_NEW_PATH) for f in file_names]
...
def read_summary_refresh():
    summary = pd.read_excel('/mnt/f/Yibing/Data/_02IPO/summary/ChiNext_rr.xlsx').rename(columns=SUMMARY_TABLE_COLUMNS_MAP_REFRESH)
    summary = summary.dropna(thresh=1)
    summary = summary[summary.SecCode.str.contains('.SZ')]
    return summary.set_index('SecCode')

def read_sub_ratio():
    data = pd.read_excel('/mnt/f/Yibing/Data/_02IPO/summary/SubRatio.xlsx').rename(columns=SUBSCRIPTION_RATIO_COLUMNS)
