...
def build_inv_type_ratio(data, summary, column="IPOPrice"):
    ...
    inv_type = (
        cur_data.groupby(["INV_TYPE", "SecCode", "IPOListDt"])
        .apply(lambda x: x["RATIO"].mean())
        .rename("Mean")
        .reset_index()
    )
    return inv_type
...
def build_4num_ratio(mean_data, summary, column="IPOPrice"):
    ...
    mean_table = (
        mean_data[
            [
                "OflBidWAvgAll",
                "OflBidWAvgMF_SS_PF_EA_Ins_QFII",
                "OflBidMedAll",
                "OflBidMedMF_SS_PF_EA_Ins_QFII",
            ]
        ]
        .div(summary[column], axis=0)
        .dropna(thresh=1)
    )
...
def build_high_low_ratio(offline_data, summary, column="IPOPrice"):
    cur_data = merge_data(
        offline_data[["OflBidLowLmtYuan", "OflBidUpLmtYuan"]],
        summary,
        ["IPOListDt", column],
    )
...
def check_valid(data, summary, offline_data):
    ...
    cur_data = merge_data(
        cur_data, offline_data, ["OflBidLowLmtYuan", "OflBidUpLmtYuan"]
    )
    cur_data["valid"] = cur_data["PRICE"].le(cur_data["OflBidUpLmtYuan"]) & cur_data[
        "PRICE"
    ].ge(cur_data["OflBidLowLmtYuan"])
    return cur_data
