...
def compute_outflow(df):
    ...
    retail_deposit = (
        nd(TRANSACTION_IND_COR) * S1_RETAIL
        + nd(TIME_DEPOSIT_LESS_250K_LESS_3M) * S1_1M_Q
        + nd(SAVING_DEPOSIT) * S1_RETAIL
    )
    ...
    operational_deposit = (
        nd(TRANSACTION_IND_COR) * (1 - S1_RETAIL)
        + nd(TRANSACTION_DOM_GOV)
        + nd(TRANSACTION_SUB_GOV)
        + nd(TRANSACTION_DOM_BNK)
        + nd(TRANSACTION_FOR_BNK)
        + nd(TRANSACTION_FOR_GOV)
        + nd(DEPOSIT_FOREIGN_OFC) * S1_1M_Y
    )
...
all_dfs = [
    pd.read_csv(
        ...
        sep="\t",
        low_memory=False,
        converters={"IDRSSD": str},
        skiprows=lambda x: x in [1],
    ).set_index("IDRSSD")
    for file in files
    if "09302024" in file and "Schedule" in file
]
